...
const basePath = 'hr/HRManagement/deductions'

export function getDeductions(params?: BonusDeductionQueryParams, signal?: AbortSignal) {
  let query = ''
  if (params) {
...
export function createDeduction(payload: BonusDeductionCreateDto, signal?: AbortSignal) {
  return apiRequest<unknown>({ method: 'POST', path: basePath, body: payload, signal }).then((r) =>
    unwrapApiResponse<BonusDeductionReadDto>(r)
  )
}

export function updateDeduction(id: string, payload: BonusDeductionUpdateDto, signal?: AbortSignal) {
  return apiRequest<unknown>({ method: 'PUT', path: `${basePath}/${id}`, body: payload, signal }).then((r) =>
    unwrapApiResponse<BonusDeductionReadDto>(r)
  )
}

export function deleteDeduction(id: string, signal?: AbortSignal) {
