...
interface EditUnitModalProps {
  isOpen: boolean;
  unit: LaundryInventoryUnitReadDto | null;
  onConfirm: (id: number, name: string) => void;
  onCancel: () => void;
  isSaving: boolean;
}

export function EditUnitModal({ isOpen, unit, onConfirm, onCancel, isSaving }: EditUnitModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (name.trim()) onConfirm(unit.id, name.trim());
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 p-6">
        ...
        <h2 className="text-[18px] font-bold text-slate-800 text-center mb-1">Edit Unit</h2>
        <p className="text-[13px] text-slate-400 text-center mb-5">Update the name for this unit.</p>
