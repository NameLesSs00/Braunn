...
import { updateLaundryInventoryItem } from '../../../../features/Laundryfeatures/laundryInventoryItems/laundryInventoryItemsSlice';
...
interface EditItemModalProps {
  isOpen: boolean;
  item: LaundryInventoryItemReadDto | null;
  onClose: () => void;
}
...
export function EditItemModal({ isOpen, item, onClose }: EditItemModalProps) {
  ...
  useEffect(() => {
    ...
    if (isOpen && item) {
      ...
      setIsFetching(true);
      getLaundryInventoryItemById(item.id)
        .then((fullItem) => {
          if (!active) return;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsSaving(false);
    onClose();
    if (updateLaundryInventoryItem.fulfilled.match(result)) {
      showSuccess('Item updated successfully');
    } else {
      showError((result.payload as string) || 'Failed to update item');
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between bg-white shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[13px] text-slate-400">Update laundry inventory item details</p>
            </div>
          </div>
        ...
        <div className="p-6 overflow-y-auto">
          <form id="laundry-edit-item-form" onSubmit={handleSubmit} className="space-y-6">
