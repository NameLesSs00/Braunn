...
interface LowStockCardProps {
  item: LaundryInventoryItemReadDto;
  onRequestClick: () => void;
}

function LowStockCard({ item, onRequestClick }: LowStockCardProps) {
  const needed =
    item.minimumStock && item.minimumStock > item.quantity
    ...
    <div className="bg-white rounded-xl border border-slate-200 p-6 flex flex-col shadow-sm">
      ...
      <div className="flex flex-col gap-3 mb-8">
        <div className="flex justify-between items-center text-[14px]">
          ...
          <span className="font-bold text-[#ef4444]">{item.quantity} {item.unitName}</span>
        </div>
        <div className="flex justify-between items-center text-[14px]">
          <span className="text-slate-500">Minimum:</span>
          <span className="font-bold text-slate-800">{item.minimumStock || '-'} {item.unitName}</span>
        </div>
        <div className="flex justify-between items-center text-[14px]">
          <span className="text-slate-500">Needed:</span>
          <span className="font-bold text-[#eab308]">{needed} {item.unitName}</span>
        </div>
      </div>
...
interface LaundryLowStockTabProps {
  items: LaundryInventoryItemReadDto[];
}

export function LaundryLowStockTab({ items }: LaundryLowStockTabProps) {
  const [isReorderOpen, setIsReorderOpen] = useState(false);
  const [selectedItem, setSelectedItem] = useState<LaundryInventoryItemReadDto | undefined>(undefined);
