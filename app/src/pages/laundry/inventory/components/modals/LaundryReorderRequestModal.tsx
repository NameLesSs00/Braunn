...
interface LaundryReorderRequestModalProps {
  isOpen: boolean;
  onClose: () => void;
...
interface PurchaseItemState {
  id: number;
  selectedItem?: LaundryInventoryItemReadDto;
  quantity: number;
}

let nextId = 0;

export function LaundryReorderRequestModal({ isOpen, onClose, initialItem }: LaundryReorderRequestModalProps) {
  const dispatch = useAppDispatch();
  const [requestDate, setRequestDate] = useState(() => new Date().toISOString().split('T')[0]);
  const [supplierName, setSupplierName] = useState('');
  const [notes, setNotes] = useState('');
  const [purchaseItems, setPurchaseItems] = useState<PurchaseItemState[]>([]);
  const [allItems, setAllItems] = useState<LaundryInventoryItemReadDto[]>([]);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (initialItem) {
        const needed =
          initialItem.minimumStock && initialItem.minimumStock > initialItem.quantity
            ? initialItem.minimumStock - initialItem.quantity
            : 1;
        setPurchaseItems([{ id: nextId++, selectedItem: initialItem, quantity: needed }]);
      } else {
        setPurchaseItems([]);
      }

      // Fetch all laundry inventory items for the dropdown
      getLaundryInventoryItems({ PageSize: 1000 })
  ...
  const handleItemChange = (id: number, itemId: number) => {
    ...
    setPurchaseItems((prev) =>
      prev.map((i) => (i.id === id ? { ...i, selectedItem: item } : i))
    );
  };
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Reorder Request" maxWidth="max-w-4xl">
      <div className="p-8">
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-[#f8fafc] border-b border-slate-200">
                    <th className="px-4 py-4 text-[12px] font-bold text-slate-500 w-[35%]">
                    ...
                    <th className="px-4 py-4 text-[12px] font-bold text-slate-500 text-center">
