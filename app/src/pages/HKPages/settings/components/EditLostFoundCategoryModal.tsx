...
interface EditLostFoundCategoryModalProps {
  isOpen: boolean;
  category: LostFoundCategoryReadDto | null;
  onConfirm: (id: number, name: string) => void;
  onCancel: () => void;
  isSaving: boolean;
}

export function EditLostFoundCategoryModal({ isOpen, category, onConfirm, onCancel, isSaving }: EditLostFoundCategoryModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (name.trim()) onConfirm(category.id, name.trim());
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 p-6">
        ...
        <h2 className="text-[18px] font-bold text-slate-800 mb-1">Edit L&F Category</h2>
        <p className="text-[13px] text-slate-400 mb-5">Update the name of this Lost &amp; Found category.</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="e.g. Phones, Wallets…"
