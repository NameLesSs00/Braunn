...
import {
  deleteHkmInventoryCategory,
  updateHkmInventoryCategory,
} from '../../../../features/HKfeatures/hkmInventoryCategories/hkmInventoryCategoriesSlice';
import type { HkmInventoryCategoryReadDto } from '../../../../models/HKmodels/HkmInventoryCategory';
...
export function CategoriesTable({ categories, isLoading, error }: CategoriesTableProps) {
  const dispatch = useAppDispatch();

  const [deleteTarget, setDeleteTarget] = useState<HkmInventoryCategoryReadDto | null>(null);
  const [editTarget, setEditTarget] = useState<HkmInventoryCategoryReadDto | null>(null);
  ...
  const handleDeleteConfirm = async () => {
    ...
    setIsDeleting(true);
    await dispatch(deleteHkmInventoryCategory(deleteTarget.id));
    setIsDeleting(false);
  ...
  const handleEditConfirm = async (id: number, name: string) => {
    setIsSaving(true);
    await dispatch(updateHkmInventoryCategory({ id, payload: { name } }));
    setIsSaving(false);
    setEditTarget(null);
  };
  ...
  return (
    <>
      ...
      {/* Delete confirmation — reuses the shared modal */}
      <DeleteConfirmModal
        isOpen={!!deleteTarget}
      ...
      {/* Edit modal — reuses the shared modal */}
      <EditUnitModal
        isOpen={!!editTarget}
        unit={editTarget}
