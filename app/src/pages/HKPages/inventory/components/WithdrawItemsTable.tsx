...
import { HkmInventoryItemReadDto, HkmInventoryItemStatus } from '../../../../models/HKmodels/HkmInventoryItem';

// Status badge: 'Low' = red pill, 'Normal' = blue pill, 'High' = orange pill
function StatusBadge({ status }: { status: HkmInventoryItemStatus }) {
  const styles = {
...
// Current stock with mini progress bar
function StockCell({ item }: { item: HkmInventoryItemReadDto }) {
  const max = item.maximumStock || 1;
  const percent = Math.min((item.quantity / max) * 100, 100);
  const isLow = item.status === 'Low';
  return (
    <div className="flex flex-col gap-1.5 min-w-[90px]">
      <span className={`text-[14px] font-bold ${isLow ? 'text-slate-800' : 'text-slate-800'}`}>
...
export function WithdrawItemsTable() {
  const dispatch = useAppDispatch();
  const { items: allItems } = useAppSelector((state) => state.hkmInventoryItems);
  const { items: issues, status: issuesStatus } = useAppSelector((state) => state.hkmIssues);

  const [currentPage, setCurrentPage] = useState(1);
  const [withdrawOpen, setWithdrawOpen] = useState(false);
  const [selectedItemForWithdraw, setSelectedItemForWithdraw] = useState<HkmInventoryItemReadDto | undefined>(undefined);
  const [reorderOpen, setReorderOpen] = useState(false);
  const [selectedItemForReorder, setSelectedItemForReorder] = useState<HkmInventoryItemReadDto | undefined>(undefined);

  useEffect(() => {
    dispatch(fetchHkmIssues());
  }, [dispatch]);

  // Compute items that were used (withdrawn) today
  const withdrawnTodayItems = useMemo(() => {
    const today = new Date().toDateString();
    
    // 1. Get issues created today
    const todayIssues = issues.filter(issue => {
      const issueDate = new Date(issue.createdAt).toDateString();
      return issueDate === today;
    });

    // 2. Extract unique item IDs
    const usedItemIds = new Set(todayIssues.map(i => String(i.itemId)));

    // 3. Filter allItems to only include those in the Set
    return allItems.filter(item => usedItemIds.has(String(item.id)));
  }, [allItems, issues]);

  const itemsPerPage = 10;
  const totalItems = withdrawnTodayItems.length;
  const totalPages = Math.ceil(totalItems / itemsPerPage) || 1;
  const startIndex = (currentPage - 1) * itemsPerPage;
  const currentData = withdrawnTodayItems.slice(startIndex, startIndex + itemsPerPage);
  ...
  const isLoading = issuesStatus === 'loading';
          ...
          <table className="w-full text-left border-collapse min-w-[800px]">
            <thead>
              <tr className="bg-[#f8fafc] border-b border-slate-200">
                {['Item', 'Category', 'Min / Max', 'Status', 'Current Stock', 'Actions'].map((col) => (
                  <th
                    key={col}
            ...
            <tbody className="divide-y divide-slate-100">
              {isLoading && currentData.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-6 py-10 text-center text-[14px] text-slate-500">
                    Loading withdrawn items...
                  </td>
                </tr>
              ) : currentData.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-6 py-10 text-center text-[14px] text-slate-500">
                    No items have been withdrawn today.
                  </td>
                </tr>
              ) : (
                      ...
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <button
                          ...
                          className={`px-5 py-2 text-[12px] font-semibold text-white rounded-xl transition-colors ${
                            item.status === 'Low' ? 'bg-[#d32f2f] hover:bg-red-700' : 'bg-slate-200 cursor-not-allowed text-slate-400'
                          }`}
