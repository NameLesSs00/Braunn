...
import { createCorporateCancellationPolicy } from '../../../features/policies/corporateCancellationPoliciesSlice'
import type { CreateCorporateCancellationPolicyDto } from '../../../models/CorporateCancellationPolicy'
...
const emptyForm: CreateCorporateCancellationPolicyDto = {
  name: '',
  code: '',
  isActive: true,
  appliesToContractType: 'Allotment',
  effectiveFrom: '',
  effectiveTo: null,
  liquidatedDamagesPenaltyPercentage: 0,
}

export function AddCorporateCancellationPolicyPopup({ onClose }: Props) {
  const dispatch = useAppDispatch()
  const [form, setForm] = useState<CreateCorporateCancellationPolicyDto>(emptyForm)
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      await dispatch(createCorporateCancellationPolicy(form)).unwrap()
      appAlert.fire({ icon: 'success', title: 'Policy Created', timer: 1500, showConfirmButton: false })
      onClose()
    } catch (err) {
      appAlert.fire('Error', err instanceof Error ? err.message : 'Failed to create policy', 'error')
    } finally {
      setSubmitting(false)
    }
  }
    ...
    <Modal open onClose={onClose} lockScroll>
      <div className="bg-white rounded-2xl w-full max-w-2xl flex flex-col max-h-[90vh] overflow-hidden shadow-2xl">
        ...
        <div className="bg-[#004bb4] px-7 py-6 text-white flex items-start justify-between rounded-t-2xl shrink-0">
          <div>
            <h2 className="text-xl font-bold tracking-tight">Add Corporate Cancellation Policy</h2>
            <p className="text-blue-200 mt-1 text-sm">Create a new corporate cancellation constraint</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto px-7 py-6 custom-scrollbar bg-slate-50/30">
          <form id="create-policy-form" onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              ...
              <div className="sm:col-span-2 pt-2">
                <label className="flex items-center gap-3 cursor-pointer select-none">
                  ...
                  <span className="text-sm font-semibold text-slate-700">Active upon creation</span>
                </label>
              </div>
            </div>
          </form>
