...
export function EditCorporateAccountPopup({ account, onClose }: EditCorporateAccountPopupProps) {
  ...
  const [formData, setFormData] = useState<UpdateCorporateAccountRequest>({
    ...
    contractEndDate: account.contractEndDate?.split('T')[0] || '',
    negotiatedRate: account.negotiatedRate || 0,
    discountPercentage: account.discountPercentage || 0,
    creditLimit: account.creditLimit || 0,
    paymentTerms: account.paymentTerms || 'Net 30 day',
    billingMethod: account.billingMethod || 'Monthly invoice',
    cancellationPolicy: account.cancellationPolicy || 'Flexible (24h free cancellation)',
  ...
  const handleSubmit = async () => {
    ...
    try {
        ...
        Swal.fire('Success', 'Corporate account updated successfully!', 'success');
        onClose();
    } catch (err: any) {
        Swal.fire('Error', err || 'Failed to update corporate account.', 'error');
    } finally {
    ...
    <Modal open onClose={onClose} lockScroll>
      <div className="bg-white rounded-xl w-full max-w-3xl flex flex-col max-h-[90vh] overflow-hidden shadow-2xl">
        ...
        <div className="bg-[#004bb4] p-6 text-white flex items-start justify-between rounded-t-xl shrink-0">
          <div>
            <h2 className="text-xl font-bold">Edit Corporate Account</h2>
            <p className="text-blue-100 mt-1 text-sm">Update corporate client account details</p>
          </div>
          <button 
            onClick={onClose}
