...
import { formatMoney, getInitials } from '../../payrollUtils';
...
export function RunPayrollConfirmLeft({ employees }: Props) {
  ...
  const total = employees.reduce((sum, employee) => sum + employee.netSalary, 0);
    ...
    <div className="rounded-[20px] border border-slate-200 bg-white overflow-hidden flex flex-col h-full">
      ...
      {!collapsed && (
        <>
          ...
          <div className="grid grid-cols-[2fr_1fr_1fr_1fr] border-b border-slate-100 px-4 py-2.5">
          ...
          <div className="flex-1 overflow-y-auto max-h-[400px]">
            {filtered.map((employee) => (
              <div
                key={employee.id}
                className="grid grid-cols-[2fr_1fr_1fr_1fr] items-center border-b border-slate-50 px-4 py-3 hover:bg-slate-50/50 transition-colors"
              >
                <div className="flex items-center gap-2.5">
                  <div className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-[#0B4EA2] text-[11px] font-bold text-white">
                    {getInitials(employee.employeeName)}
                  </div>
                  <div>
                    <div className="text-[13px] font-bold text-slate-800 leading-tight">{employee.employeeName}</div>
                    <div className="text-[11px] text-slate-400">Reviewed payroll</div>
                  </div>
                </div>

                <div className="text-[14px] font-bold text-slate-800 text-right">{formatMoney(employee.netSalary)}</div>
