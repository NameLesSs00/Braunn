import { Search, ChevronDown, Download, CheckCircle2 } from 'lucide-react';
...
type Props = {
  ...
  setDetailsRecord: (record: ReviewPayrollRecord) => void;
};
            ...
            {pageRecords.map((rec) => (
              <tr key={rec.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                ...
                <td className="px-4 py-5">
                  <div className="flex items-center gap-3">
                    <div className="grid h-10 w-10 place-items-center rounded-xl text-[12px] font-bold text-white shadow-sm" style={{ backgroundColor: rec.color }}>
                      {rec.initials}
                    </div>
                    <div>
                ...
                <td className="px-4 py-5 text-[14px] text-slate-600">{rec.department}</td>
                <td className="px-4 py-5 text-center text-[14px] text-slate-600">{rec.basicSalary.toLocaleString()}$</td>
                <td className="px-4 py-5 text-center text-[14px] text-slate-600">{rec.allowances.toLocaleString()}$</td>
                <td className="px-4 py-5 text-center text-[14px] font-medium text-red-500">{rec.deductions.toLocaleString()}$</td>
                <td className="px-4 py-5 text-center text-[14px] text-slate-600">{rec.grossPay.toLocaleString()}$</td>
                <td className="px-4 py-5 text-center text-[15px] font-bold text-slate-900">{rec.netPay.toLocaleString()}$</td>
                <td className="px-4 py-5 text-center">
                  {rec.status === 'REVIEWED' ? (
                    ...
                    <span className="inline-flex rounded-full bg-yellow-100 px-2.5 py-1 text-[11px] font-bold text-yellow-700 tracking-wide">
                      PENDING
                    </span>
                  )}
                </td>
