...
const navItems: NavItem[] = [
  ...
  { to: routes.hk.inventory, label: 'Inventory', icon: FiBox },
  { to: routes.hk.settings, label: 'Settings', icon: IoSettingsOutline },
]

function HousekeepingSidebarLink({ item }: { item: NavItem }) {
  return (
    <NavLink
      to={item.to}
      className={({ isActive }) =>
    ...
    >
      ...
      <div className="grid grid-cols-[44px_1fr] flex-shrink-0">
        ...
        <div className="flex flex-col justify-center px-6 py-5 border-b border-slate-100">
          ...
          <p className="mt-1 text-[11px] font-medium text-[#0B4EA2]">Housekeeping Department</p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden pb-4">
        <nav className="flex flex-col mt-2">
          {navItems.map((item) => (
            <HousekeepingSidebarLink key={item.to} item={item} />
          ))}
        </nav>
      </div>
    </aside>
